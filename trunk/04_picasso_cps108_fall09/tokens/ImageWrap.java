...
public class ImageWrap extends MultiArgToken {
    ...
    protected void calculateCoords(double x, double y)
    {
        ...
        if (x < Coordinates.DOMAIN_MIN) x += range;
        else if (x > Coordinates.DOMAIN_MAX) x -= range;
        if (y < Coordinates.DOMAIN_MIN) y += range;
        else if (y > Coordinates.DOMAIN_MAX) y -= range;
        shiftCoords(x, y);
    }
